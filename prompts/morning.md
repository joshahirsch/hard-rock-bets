...
Read the Decision Log tab and the Current Bankroll figure.
...
One call per candidate, carrying your judgments. It returns, in one response:
...
7. Decision Log rows appended via the service's returned cells. Columns L and M
   stay **empty** — the sheet's own formulas own them.
